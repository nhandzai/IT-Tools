...
export default function EditTool({ toolName, toolDescription, icon, isPremium, onClose, onSave }) {
                ...
                <section className="flex sm:flex-row flex-col gap-10 justify-between">
                    <section className="flex flex-col gap-4 text-gray-500 w-full">
                            ...
                            {showIcons && (
                                <div className="flex  gap-4 mt-3 border p-3 rounded-md flex-col w-fit  -bottom-55 h-50 absolute overflow-y-auto bg-gray-50">
                                    {iconList.map((iconFile) => (
                                        <div
                                            key={iconFile}
                                            className={`border-2 p-1 rounded cursor-pointer ${selectedIcon === iconFile
                                                ? 'border-blue-500'
                                                : 'border-transparent'
                                                }`}
                                            onClick={() => {
                        ...
                        <div className="flex flex-col">
                            ...
                            <button
                                className={`mt-2 w-32 px-4 py-2 rounded-lg font-medium transition ${premium ? 'bg-yellow-400 text-white' : 'bg-gray-300 text-black'
                                    }`}
                        ...
                        <section className='flex justify-between items-center gap-4'>
                            <button className="bg-gray-400 cursor-pointer text-black font-medium p-2 rounded-lg mt-4 w-2/3 hover:bg-gray-800 transition duration-300"
                                onClick={onClose}>
                                Cancel
                            </button>
                            ...
                            <button className="bg-yellow-400 cursor-pointer text-white font-medium p-2 rounded-lg mt-4 w-2/3 hover:bg-blue-500 transition duration-300"
                                onClick={() => {
                                    onSave({
                                        toolName: document.querySelector('input[id="toolName"]').value,
                                        toolDescription: document.querySelector('input[id="toolDescription"]').value,
                                        icon: selectedIcon,
                                        isPremium: premium,
                                    });
                                    onClose();
